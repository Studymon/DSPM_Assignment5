...
We are losing about 220 rows out of the 12934 (~1.7%). This is partially due to the three faulty pages accounting for 60 lost observations. The rest is missing at the end. Multiple observations threw errors due to missing values/empty lists. Instead of making the loop more complicated, I opted for leaving the rest out, as these are relatively few observations compared to the total number.
...
We removed 247 observations with faulty coordinates
...
for (i in 1:n_pages_DK) {
  ...
  for (j in 1:tempResults[["page"]]$size) {
    ...
    ifelse("url" %in% names(tempResults[["_embedded"]][["venues"]][[j]]),
           venuesDK[x,5] <- tempResults[["_embedded"]][["venues"]][[j]]$url,
           venuesDK[x,5] <- NA)
...
We lose 4484 - 4340 = 144 rows (3.2%).
100 missing rows are due to faulty pages (faultyPages = 5)
...
We removed 177 observations with faulty coordinates
